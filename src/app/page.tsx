import Header from "@/components/layout/Header";
import LeftSide from "@/components/layout/LeftSide";
import Banner from "@/components/sections/Banner";
import About from "@/components/sections/About";
import Experience from "@/components/sections/Experience";
import Contact from "@/components/sections/Contact";
import Footer from "@/components/layout/Footer";
import RightSide from "@/components/layout/RightSide";

import Projects from "@/components/sections/Projects";

import {
  about,
  banner,
  capstoneProjects,
  contact,
  experience,
  miniProjects,
  skillGroups,
} from "@/lib/content";

export default function HomePage() {
  return (
    <div className="bg-bodyColor font-bodyFont text-textLight scrollbar-track-textDark/5 scrollbar-thumb-textDark/70 h-screen w-full scrollbar-thin overflow-x-hidden overflow-y-scroll">
      <Header />
      <div className="h-[88vh] w-full items-center justify-between gap-10 lg:flex">
        <div className="fixed bottom-0 left-0 hidden h-full w-32 xl:inline-flex">
          <LeftSide />
        </div>
        <div className="mx-auto h-[88vh] w-full p-4">
          <main>
            <Banner copy={banner} />
            <About copy={about} skills={skillGroups} />
            <Experience jobs={experience} />
            <Projects capstone={capstoneProjects} mini={miniProjects} />
            <Contact copy={contact} />
          </main>
          <Footer />
        </div>
        <div className="fixed right-0 bottom-0 hidden h-full w-32 xl:inline-flex">
          <RightSide />
        </div>
      </div>
    </div>
  );
}
